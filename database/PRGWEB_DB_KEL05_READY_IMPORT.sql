-- phpMyAdmin SQL Dump - versi siap impor
-- Kompatibel dengan phpMyAdmin pada XAMPP dan Laragon
-- Target: MySQL 5.7+/8.x dan MariaDB 10.4+
--
-- PERINGATAN:
-- Berkas ini akan menghapus database `db_simat` apabila sudah ada,
-- kemudian membuatnya kembali dari awal agar impor dapat diulang tanpa konflik.
...
CREATE PROCEDURE `usp_insert_mata_kuliah` (IN `p_nama` VARCHAR(100), IN `p_kode` VARCHAR(20), IN `p_sks` INT, IN `p_semester` INT, IN `p_id_kelas_csv` TEXT)   BEGIN
...
CREATE PROCEDURE `usp_select_laporan_histori_jam_mahasiswa_filter` (IN `p_id_pengguna` INT, IN `p_tanggal_mulai` DATE, IN `p_tanggal_selesai` DATE)   BEGIN
    SELECT
        id_transaksi,
        jenis_transaksi,
        tanggal_transaksi,
...
CREATE PROCEDURE `usp_select_laporan_pengaduan_fasilitas_filter` (IN `p_tanggal_mulai` DATE, IN `p_tanggal_selesai` DATE)   BEGIN
    SELECT
        id_pengaduan_kerusakan_fasilitas,
        nim,
        nama_mahasiswa,
...
CREATE PROCEDURE `usp_update_mata_kuliah` (IN `p_id` INT, IN `p_nama` VARCHAR(100), IN `p_kode` VARCHAR(20), IN `p_sks` INT, IN `p_semester` INT, IN `p_status` VARCHAR(20), IN `p_id_kelas_csv` TEXT)   BEGIN
    DECLARE v_csv TEXT;
    DECLARE v_token VARCHAR(30);
...
-- Fungsi
--
CREATE FUNCTION `ufn_cari_id_kelas_di_table_detail_fasilitas_pada_kelas` (`p_id_pengguna` INT) RETURNS INT(11) READS SQL DATA BEGIN
    DECLARE v_id_kelas INT;
